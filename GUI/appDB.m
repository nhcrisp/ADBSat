...
%------------ BEGIN CODE ----------%
classdef appDB < matlab.apps.AppBase
    ...
    properties (Access = public)
        ...
        AoSMinEditField                 matlab.ui.control.NumericEditField
        AoSMaxEditField                 matlab.ui.control.NumericEditField
        AoSStepEditField                matlab.ui.control.NumericEditField
        AoSDashLabel                    matlab.ui.control.Label
        AoSStepLabel                    matlab.ui.control.Label
        AngleofSideslipEditFieldLabel   matlab.ui.control.Label
        AoAMinEditField                 matlab.ui.control.NumericEditField
        AoAMaxEditField                 matlab.ui.control.NumericEditField
        AoAStepEditField                matlab.ui.control.NumericEditField
        AoADashLabel                    matlab.ui.control.Label
        AoAStepLabel                    matlab.ui.control.Label
        ...
        % Reconfigured Model Selection UI
        ...
        % Preset save/load (persistent header above the tab group)
        SavePresetButton                matlab.ui.control.Button
        LoadPresetButton                matlab.ui.control.Button
        
        % Export controls (Simulation tab)
        ...
        % Progress Bar specific to bottom left
    ...
    properties (Access = private)
        onePanelWidth = 640;
        CancelRequested logical = false; % ADDED flag to handle loop escapes
    ...
    methods (Access = public)
        ...
        % Used by RunButtonPushed, which - now that it starts from
        % Checkpoint 2 onward - no longer regenerates the mesh/quality/
        % normals/material plots, so those should be left alone rather
        % than wiped on every simulation run
        ...
        % Renders the aerodynamic database as a contour plot of AoS (x)
        % vs AoA (y) vs a chosen result field - the same plot produced at
        % the bottom of the ADBSat example live script
        function plotAedbContour(app, fileOut)
            S = load(fileOut);
            if ~isfield(S, 'aedb')
                error('Selected result file does not contain a merged aerodynamic database (was only a single AoA/AoS point run?).');
            end
            aedb = S.aedb;
            if ~isfield(aedb, 'aoa') || ~isfield(aedb, 'aos')
                error('Aerodynamic database is missing its aoa/aos grid vectors.');
            end
            
            nAoA = size(aedb.aoa, 1);
            nAoS = size(aedb.aos, 2);
            
            candidates = struct('label', {}, 'group', {}, 'field', {});
            groupNames = {'aero', 'solar'};
            for g = 1:numel(groupNames)
                gName = groupNames{g};
                if isfield(aedb, gName) && isstruct(aedb.(gName))
                    gFields = fieldnames(aedb.(gName));
                    for f = 1:numel(gFields)
                        val = aedb.(gName).(gFields{f});
                        if isnumeric(val) && isequal(size(val), [nAoA, nAoS])
                            candidates(end+1) = struct('label', sprintf('%s.%s', gName, gFields{f}), ...
                                'group', gName, 'field', gFields{f}); %#ok<AGROW>
                        end
                    end
                end
            end
            ...
            if isempty(candidates)
                error('No plottable [AoA x AoS] result fields were found in the aerodynamic database.');
            end
            
            labels = {candidates.label};
            app.PlotDropDown.Items = labels;
            if ~any(strcmp(app.PlotDropDown.Value, labels))
                app.PlotDropDown.Value = labels{1};
            end
            
            sel = candidates(strcmp(labels, app.PlotDropDown.Value));
            dataMat = aedb.(sel.group).(sel.field);
            
            cla(app.UIAxes1);
            contourf(app.UIAxes1, aedb.aos.*(180/pi), aedb.aoa.*(180/pi), dataMat);
            cb = colorbar(app.UIAxes1);
            cb.Label.String = sel.label;
            xlabel(app.UIAxes1, 'Angle of Sideslip (deg)');
            ylabel(app.UIAxes1, 'Angle of Attack (deg)');
            title(app.UIAxes1, sprintf('Aerodynamic Database: %s', sel.label), 'FontSize', 12);
        ...
        % field, or a fresh sweep) stay visually comparable instead of each
        % one auto-scaling to its own data.
        ...
        % Shared logic for re-displaying the cached aerodynamic database
        % with whichever field is currently selected in PlotDropDown,
        % without re-running the (potentially expensive) AoA/AoS sweep
        function refreshAedbPlot(app, notifyIfMissing)
            if nargin < 2
                notifyIfMissing = false;
            end
            if isempty(app.LastFileOut) || ~exist(app.LastFileOut, 'file')
                if notifyIfMissing
                    app.logMessage('No aerodynamic database is loaded yet - run a simulation first.');
                end
                return;
            end
            try
                app.plotAedbContour(app.LastFileOut);
        ...
        % Lets the user switch which result field is contoured without
        % re-running the sweep, as long as a database from a previous
        % run is still cached
        function PlotDropDownValueChanged(app, event)
            app.refreshAedbPlot(false);
        end

        % Shows/hides one of the overlaid mesh axes
    ...
    methods (Access = private)
        % opens file explorer to find input file
        ...
        % Manual reset of every plot tab back to a blank state - probably
        % obsolete now
        ...
        % Save configuration to a JSON preset file
        function SavePresetButtonPushed(app, event)
            ...
            % Simulation tab
            cfg.aoaMin = app.AoAMinEditField.Value;
            cfg.aoaMax = app.AoAMaxEditField.Value;
            cfg.aoaStep = app.AoAStepEditField.Value;
            cfg.aosMin = app.AoSMinEditField.Value;
            cfg.aosMax = app.AoSMaxEditField.Value;
            cfg.aosStep = app.AoSStepEditField.Value;
        ...
        % Load a JSON preset file and repopulate every configuration field
        function LoadPresetButtonPushed(app, event)
            ...
            if isfield(cfg,'aoaMin'),  app.AoAMinEditField.Value = cfg.aoaMin;   end
            if isfield(cfg,'aoaMax'),  app.AoAMaxEditField.Value = cfg.aoaMax;   end
            if isfield(cfg,'aoaStep'), app.AoAStepEditField.Value = cfg.aoaStep; end
            if isfield(cfg,'aosMin'),  app.AoSMinEditField.Value = cfg.aosMin;   end
            if isfield(cfg,'aosMax'),  app.AoSMaxEditField.Value = cfg.aosMax;   end
            if isfield(cfg,'aosStep'), app.AoSStepEditField.Value = cfg.aosStep; end
        ...
        function ExportPlotButtonPushed(app, event)
            ...
            if activeTab == app.PlotTab2
                % Merged Mesh tab overlays two axes - export whichever the
                % toggle switch currently shows
        ...
        % Exports every populated plot tab (database contour, mesh,
        % quality, normals, material ID) to PNG files in one folder, so
        % the user doesn't have to export each tab individually via
        % Export Plot...
        function ExportAllPlotsButtonPushed(app, event)
            ...
            exportList = { ...
                'Database Contour Plot', app.UIAxes1; ...
                'Mesh',                  meshAx; ...
        ...
        % Rebuilds the Results Summary tab from the run configuration, the
        % imported mesh stats, and any scalar fields found in the solver
        % output .mat
        function updateResultsSummary(app, modOut, fileOut, alt, inc, aoa_deg, aos_deg, inparam, shadow, solar)
            ...
            rows(end+1, :) = {'AoA Range (deg)', sprintf('%.3g to %.3g (step %.3g, %d points)', ...
                app.AoAMinEditField.Value, app.AoAMaxEditField.Value, app.AoAStepEditField.Value, numel(aoa_deg))};
            rows(end+1, :) = {'AoS Range (deg)', sprintf('%.3g to %.3g (step %.3g, %d points)', ...
                app.AoSMinEditField.Value, app.AoSMaxEditField.Value, app.AoSStepEditField.Value, numel(aos_deg))};
        ...
        function RunButtonPushed(app, event)
            ...
            aoaMinV = app.AoAMinEditField.Value;
            aoaMaxV = app.AoAMaxEditField.Value;
            aoaStepV = app.AoAStepEditField.Value;
            aosMinV = app.AoSMinEditField.Value;
            aosMaxV = app.AoSMaxEditField.Value;
            aosStepV = app.AoSStepEditField.Value;
            
            if isnan(aoaStepV) || aoaStepV <= 0
                validationErrors{end+1} = 'AoA step size must be a positive number.'; 
            end
            if isnan(aoaMinV) || isnan(aoaMaxV) || aoaMaxV < aoaMinV
                validationErrors{end+1} = 'AoA maximum must be greater than or equal to AoA minimum.'; 
            end
            if isnan(aosStepV) || aosStepV <= 0
                validationErrors{end+1} = 'AoS step size must be a positive number.'; 
            end
            if isnan(aosMinV) || isnan(aosMaxV) || aosMaxV < aosMinV
                validationErrors{end+1} = 'AoS maximum must be greater than or equal to AoS minimum.';
            end
            ...
            aoa_deg = aoaMinV:aoaStepV:aoaMaxV;
            aos_deg = aosMinV:aosStepV:aosMaxV;
            if isempty(aoa_deg), aoa_deg = aoaMinV; end
            if isempty(aos_deg), aos_deg = aosMinV; end
            ...
            % Calculate aerodynamic database over the AoA x AoS sweep
            app.logMessage(sprintf('Computing aerodynamic database over %d AoA x %d AoS combinations...', numel(aoa_deg), numel(aos_deg)));
            try
                inparam = environment(inparam, env(1), env(2), env(3), env(4), env(5), env(6), env(7), env(8:14), env(15));

                fileOut = ADBSatFcn(modOut, resOut, inparam, aoa_deg, aos_deg, shadow, solar, 0, del, 0);
                app.logMessage('Aerodynamic database evaluation routines mapped.');
            catch ME
            ...
            % -----------------------------------------------------------
            % 3. PLOT AERODYNAMIC DATABASE CONTOUR
            % (replaces the 3D surface response plot - mirrors the
            % contourf(aedb.aos, aedb.aoa, aedb.aero.Cf_wX) plot at the
            % bottom of the ADBSat example live script)
            % -----------------------------------------------------------
            if verb && ~del
                app.logMessage('Building aerodynamic database contour plot...');
                try
                    app.plotAedbContour(fileOut);
        ...
        function ProcessGeometryButtonPushed(app, event)
            ...
            if isnan(elementSizeVal) || elementSizeVal <= 0
                validationErrors{end+1} = 'Element Size (m) must be a positive number.';
            end
            if isnan(subdivisionsVal) || subdivisionsVal < 0 || mod(subdivisionsVal, 1) ~= 0
                validationErrors{end+1} = 'Number of Subdivisions must be a non-negative whole number.'; 
            end
    ...
    methods (Access = private)
        function createComponents(app)
            ...
            app.UIFigure.Name = 'ADBSat Aerodynamic Database Toolkit App';
            ...
            app.SelectedPathLabel.Position = [10 670 200 22]; 
            ...
            app.ProgressBar.Position = [10 10 315 25];
            ...
            app.AngleofAttackEditFieldLabel.Position = [10 630 85 22];
            app.AngleofAttackEditFieldLabel.Text = 'AoA (deg):';
            
            app.AoAMinEditField = uieditfield(simControlPanel, 'numeric');
            app.AoAMinEditField.Limits = [-360 360];
            app.AoAMinEditField.Position = [100 630 55 22];
            app.AoAMinEditField.Value = -45;
            app.AoAMinEditField.Tooltip = 'Minimum angle of attack (deg)';
            
            app.AoADashLabel = uilabel(simControlPanel);
            app.AoADashLabel.HorizontalAlignment = 'center';
            app.AoADashLabel.Position = [157 630 12 22];
            app.AoADashLabel.Text = '-';
            
            app.AoAMaxEditField = uieditfield(simControlPanel, 'numeric');
            app.AoAMaxEditField.Limits = [-360 360];
            app.AoAMaxEditField.Position = [171 630 55 22];
            app.AoAMaxEditField.Value = 45;
            app.AoAMaxEditField.Tooltip = 'Maximum angle of attack (deg)';
            
            app.AoAStepLabel = uilabel(simControlPanel);
            app.AoAStepLabel.HorizontalAlignment = 'center';
            app.AoAStepLabel.Position = [228 630 20 22];
            app.AoAStepLabel.Text = char(hex2dec('0394'));
            
            app.AoAStepEditField = uieditfield(simControlPanel, 'numeric');
            app.AoAStepEditField.Limits = [0.0001 360];
            app.AoAStepEditField.Position = [250 630 65 22];
            app.AoAStepEditField.Value = 1;
            app.AoAStepEditField.Tooltip = 'Angle of attack step size (deg)';
            
            app.AngleofSideslipEditFieldLabel = uilabel(simControlPanel);
            app.AngleofSideslipEditFieldLabel.HorizontalAlignment = 'right';
            app.AngleofSideslipEditFieldLabel.Position = [10 590 85 22];
            app.AngleofSideslipEditFieldLabel.Text = 'AoS (deg):';
            
            app.AoSMinEditField = uieditfield(simControlPanel, 'numeric');
            app.AoSMinEditField.Limits = [-360 360];
            app.AoSMinEditField.Position = [100 590 55 22];
            app.AoSMinEditField.Value = -45;
            app.AoSMinEditField.Tooltip = 'Minimum angle of sideslip (deg)';
            
            app.AoSDashLabel = uilabel(simControlPanel);
            app.AoSDashLabel.HorizontalAlignment = 'center';
            app.AoSDashLabel.Position = [157 590 12 22];
            app.AoSDashLabel.Text = '-';
            
            app.AoSMaxEditField = uieditfield(simControlPanel, 'numeric');
            app.AoSMaxEditField.Limits = [-360 360];
            app.AoSMaxEditField.Position = [171 590 55 22];
            app.AoSMaxEditField.Value = 45;
            app.AoSMaxEditField.Tooltip = 'Maximum angle of sideslip (deg)';
            
            app.AoSStepLabel = uilabel(simControlPanel);
            app.AoSStepLabel.HorizontalAlignment = 'center';
            app.AoSStepLabel.Position = [228 590 20 22];
            app.AoSStepLabel.Text = char(hex2dec('0394')); % Delta
            
            app.AoSStepEditField = uieditfield(simControlPanel, 'numeric');
            app.AoSStepEditField.Limits = [0.0001 360];
            app.AoSStepEditField.Position = [250 590 65 22];
            app.AoSStepEditField.Value = 1;
            app.AoSStepEditField.Tooltip = 'Angle of sideslip step size (deg)';
            ...
            % Placeholder items - replaced automatically after the first
            % run with whatever [AoA x AoS] fields the aerodynamic
            % database actually contains (see plotAedbContour).
            app.PlotDropDown.Items = {'aero.Cf_wX', 'aero.Cf_wY', 'aero.Cf_wZ', 'aero.Cm_BX', 'aero.Cm_BY', 'aero.Cm_BZ'};
            app.PlotDropDown.Position = [130 210 190 22];
            app.PlotDropDown.Value = 'aero.Cf_wX';
            ...
            app.PlotTab1.Title = 'Database Contour Plot';
